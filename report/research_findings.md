# 資料增強與影像 corruption robustness：受控比較與 CutMix 使用頻率分析

定稿候選版，2026-09-17。共完成 27 組訓練：18 組 CIFAR-10 與 9 組 CIFAR-100；中斷封存的 run 不計入。本文定位為受控實證比較與設定敏感性分析，並未提出新 augmentation 方法。
...
目前已足以形成階段成果，不需要為完成本報告追加訓練。若需擴大結論範圍，應事先固定第二模型或更不同的資料來源及評估規則；若要解釋機制，則需直接測量與干預設計。不能只靠更多 accuracy 表格推論高頻依賴或因果關係。三 seeds、同一 CIFAR 家族及不同切分／正規化流程仍限制外推。
